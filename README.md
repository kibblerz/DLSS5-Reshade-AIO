...
Vulkan borderless games may keep a native-size swapchain even when their configuration names a lower resolution. Enable `Force reduced Vulkan swapchain extent` and enter the desired render width and height in the addon panel (or set `VulkanForceRenderResolution=1`, `VulkanRenderWidth`, and `VulkanRenderHeight` under `[Standalone.DLSSNR]` in `ReShade.ini`). The addon changes only the Vulkan render extent; the game window and D3D12 presentation proxy remain at the monitor's native size. Restart the game or recreate its swapchain after changing this setting.

Vulkan frame copies are recorded during ReShade's `present`/`reshade_present` callbacks but deliberately submitted by ReShade itself with the game's render-complete wait semaphores. The D3D12 timeline handoff, NGX evaluation, and native proxy presentation occur at `finish_present`. Do not move the Vulkan path back to a manual immediate-list flush inside `present`; No Man's Sky device-loses that unsynchronized submission and displays black.
...
Run `lab\build-vulkan-wsi-smoke.bat` to report the Win32 surface extent and verify whether the current Vulkan driver accepts a reduced 1920x1080 swapchain for a native 3840x2160 borderless client. This isolates render-extent support from NGX and the game.
