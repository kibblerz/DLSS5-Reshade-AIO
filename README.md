...
| Frame Generation says `off/safety` | The game loaded its own NVIDIA Streamline presentation hook. The addon disables its FG stage to prevent two swap-chain/FG systems from deadlocking. NR and DLSS/DLAA still work. The ReShade panel exposes an unsafe expert override, but do not enable it while changing resolution or display mode. |
