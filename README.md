...
Optical Flow may also increase perceived input latency even when the displayed FPS remains high. The asynchronous pipeline can retain one or more captured frames while motion analysis, NR, DLSS, FG, and presentation complete; at a 30 FPS source cadence, every additional queued frame is about 33 ms old. If controls feel noticeably delayed after enabling Optical Flow, disable **NVIDIA Optical Flow motion** for that game. This persistent per-game opt-out is the current low-latency workaround while freshness-first motion scheduling is developed.
...
| **Controls feel delayed with Optical Flow enabled, despite normal FPS** | Disable **NVIDIA Optical Flow motion** for that game. Asynchronous motion analysis can make the displayed frame older without lowering the FPS counter, especially with a 30 FPS source cap, multiple NR passes, FG, or VSync. The opt-out persists across launches. |
...
- Adds an independent, aspect-preserving NVIDIA Optical Flow working-resolution selector.
- Makes **Auto (cap at 720p)** the recommended default after testing found negligible quality improvement at 1080p in Conan Exiles.
- Keeps Native, 1440p, 1080p, and 720p modes available for per-game quality/performance tuning.
- Downsamples only NVOF input, then reconstructs motion and confidence at the full NR/DLSS/FG guide resolution with correctly scaled vector magnitude.
- Safely defers Optical Flow reconfiguration until staged D3D11 submissions finish consuming provider resources, preventing the crash previously seen when disabling Optical Flow live.
