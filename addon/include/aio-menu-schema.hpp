...
inline constexpr const char *kNvofResolutionItems[] = {
    "Auto (cap at 180p)", "Native source resolution", "Cap at 1440p",
...
inline constexpr NRSetting kSettings[] = {
    ...
    { "NvidiaOpticalFlowResolution", "Optical Flow working resolution", NR_COMBO, 0, 0, 6, nullptr, kNvofResolutionItems, 7,
      "Auto preserves the source aspect ratio and caps Optical Flow at 180p to minimize pipeline backpressure. Raise it if a game needs more thin-object motion precision; motion and confidence are reconstructed at the NR source resolution.", Group::Neural },
    ...
    { "NvidiaOpticalFlowMotionRepair", "Geometry object-fill strength", NR_FLOAT, 1, 0, 1, "%.2f", nullptr, 0,
      "Repairs motion and confidence inside depth objects, including the foreground side of thin silhouettes. Zero preserves the original data; one applies full repair.", Group::Neural },
    { "NvidiaOpticalFlowSubmitDepth", "Submit captured ReShade depth directly to NGX (experimental)", NR_BOOL, 0, 0, 1, nullptr, nullptr, 0,
      "Off by default. Geometry-aware motion repair remains active while raw ReShade depth submission to NR, DLSS, and Frame Generation is disabled.", Group::Neural },
    { "NvidiaOpticalFlowConsistency", "Flow disagreement tolerance", NR_FLOAT, 3, 0.5f, 12, "%.1f px", nullptr, 0,
      "Raise only when moving geometry is red because forward and backward motion disagree. Too high may trust incorrect motion.", Group::Neural },
    { "NvidiaOpticalFlowCost", "Low-texture confidence tolerance", NR_FLOAT, 0.35f, 0, 0.99f, "%.2f", nullptr, 0,
      "Raise when dark or textureless moving areas remain red. Too high may trust uncertain Optical Flow data. This affects DLSS history rejection, not NR strength.", Group::Neural },
