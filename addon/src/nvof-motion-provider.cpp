...
bool NvofMotionProvider::CreatePipelineState()
{
    ...
    static const char conversion_shader[] =
        ...
        "float2 repaired=f;float bestScore=rejection;"
        "if(UseDepth!=0&&valid>0.5&&MotionRepairStrength>0.0){"
        "static const int2 offsets[12]={int2(-2,0),int2(2,0),int2(0,-2),int2(0,2),"
        "int2(-4,0),int2(4,0),int2(0,-4),int2(0,4),int2(-4,-4),int2(4,-4),int2(-4,4),int2(4,4)};"
        "[unroll]for(int i=0;i<12;i++){int2 candidate=int2(id.xy)+offsets[i];"
        ...
        "float surfacePenalty=smoothstep(DepthSensitivity,DepthSensitivity*4.0,depthGap)*2.0;"
        "float score=(candidateOutside?1.0:candidateBad)+surfacePenalty;"
        "if(score<bestScore){bestScore=score;repaired=candidateFlow;}}}"
        "float repairBlend=saturate(MotionRepairStrength*rejection);"
