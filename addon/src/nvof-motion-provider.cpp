...
bool NvofMotionProvider::CreatePipelineState()
{
    ...
    static const char conversion_shader[] =
        ...
        "float weight=reliability*(1.0-surfaceMismatch)*distanceWeight;"
        ...
        "if(bestScore<rejection)repaired=bestFlow;"
        ...
        "float coverage=smoothstep(0.5,2.0,coverageGap)*smoothstep(0.55,0.82,coherence)*"
        "smoothstep(0.5,2.0,motionWeight);"
        "repaired=lerp(repaired,consensus,coverage);rejection=max(rejection,coverage);}"
